...
export const useWebFont = (props: WebFontProps[]): void => {
  props.forEach(
    ({ baseUrl, fontName, fontFamily, isDefault, weights, style }) => {
      const uid = `${fontName}-${(weights || []).join("-")}${
        style ? `-${style}` : ""
      }`;
      ...
      let fontDefinition: Record<string, string> = {};
      if (Array.isArray(weights) && weights.length > 0) {
        weights.forEach((x) => {
          const weigth = FontWeigths[x];
          fontDefinition[
            weigth
          ] = `@font-face { font-family: "${fontFamily}"; src: local("${fontFamily} ${weigth}${
            style ? ` ${style}` : ""
        ...
        ["woff", "woff2"].forEach((format: string) => {
          ...
          if (!domLink) {
            ...
            if (domLink instanceof HTMLLinkElement) {
              ...
              domLink[
                "href"
              ] = `${baseUrl}/${fontName}-${FontWeigths.Regular}.${format}`;
            }
          }
        });
