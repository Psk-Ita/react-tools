...
const objName = "MyImage";
const cssClassName = "my-image";
    ...
    switch (imageId) {
        ${_options.map((x) => {
          let props = `(oth as React.SVGProps<${
            svgfiles.indexOf(`${ICONS_SOURCE_DIR}/${x}.svg`) < 0
              ? "HTMLImageElement"
              : "SVGSVGElement"
          }>)`;
          return `case ${objName}sEnum.${clean(x)}: {
...
export default {${objName}, ${objName}sEnum, ${_exports.join(`,
    `)},};
`,
  function (err) {
